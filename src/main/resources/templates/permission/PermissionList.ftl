...
<script>
    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
        ...
        table.render({
            elem: '#table',//table id
            url: '${context.contextPath}/permission/list',
            method: 'POST', //方式
            page: true,//是否开启分页
...
//                },
        ...
        function openAdd() {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['permissionInfoAdd.jsp', 'yes'], //iframe的url，no代表不显示滚动条
        ...
        function openDetail(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['permissionInfoDetail.jsp', 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
                    var body = layer.getChildFrame('body', lockIndex);
                    //绑定解锁按钮的点击事件
                    body.find('button#close').on('click', function () {
       ...
       /* function openUpdate(data) {
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean: true,
                content: ['toUrl?url=/permission/permissionInfoEdit&obj=' + encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function (layero, lockIndex) {
//                    alert(JSON.stringify(data));
            ...
            layer.open({
                ...
                , success: function (layero) {
                    ...
                    btn.find('.layui-layer-btn0').on('click', function () {
                        ...
                        $.ajax({
                            url: '${context.contextPath}/permission/delete',
                            data: {
                                id: ids
                            },
        ...
        $('button#treeSelect').on('click', function () {
            layer.open({
                ...
                //area: ['100%', '100%'],
                boolean: true,
                content: ['permissionInfoTree.jsp', 'yes'], //iframe的url，no代表不显示滚动条
                btn: ['选择', '关闭'],
