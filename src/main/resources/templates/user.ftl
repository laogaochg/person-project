<html>
    ...
    <body>
        <#--<h1>${message}</h1>-->
        <h1>用户列表-<a href="${context.contextPath}/logout">登出</a></h1>
        <h2>权限列表</h2>
        ...
        <@shiro.hasRole name="editor">editor角色登录显示此内容</@shiro.hasRole><br/>
        ...
        <@shiro.hasPermission name="manage">add权限用户显示此内容1</@shiro.hasPermission><br/>
        <@shiro.hasPermission name="角色查询">用户编辑权限用户显示此内容1</@shiro.hasPermission><br/>
        ...
        <ul>
            <#list userList as user>
                <li>用户名：${user.nickname}---密码：${user.pswd}---<a href="${context.contextPath}/user/edit/${user.id}">修改用户</a></li>
            </#list>
        </ul>
    </body>
</html>
