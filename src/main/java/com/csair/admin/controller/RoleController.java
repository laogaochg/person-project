...
//    @RequiresPermissions("角色权限编辑")
    public ModelAndView editRolePermission(Long roleId,Long[] permissionIds,ModelAndView model) {
        ...
        if (roleId == null) {
            ...
            map = roleService.editRolePermission(roleId,permissionIds,user);
        }
...
//    @RequiresPermissions("角色成员查询")
    public ModelAndView queryUserByRole(RoleUserQueryObject qo,ModelAndView model) {
        RoleQueryObject uqo = new RoleQueryObject();
        uqo.setId(qo.getId());
        PageResult roleList = roleService.query(uqo);
        if (roleList.getListData().size() != 0) {
            model.addObject("role",roleList.getListData().get(0));
        }
        PageResult pageResult = roleService.queryUserByRole(qo);
        model.addObject("pageResult",pageResult);
...
//    @RequiresPermissions("角色编辑")
    public ModelAndView inputRole(Long roleId,ModelAndView model) {
        if (roleId != null) {
            ...
            qo.setId(roleId);
            PageResult roleList = roleService.query(qo);
            if (roleList.getListData().size() != 0) {
                model.addObject("role",roleList.getListData().get(0));
            }
        }
